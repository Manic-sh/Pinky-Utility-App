...
const Home = () => {
  ...
  const [IsBillerBBPS, setIsBillerBBPS] = useState(false);
  ...
  async function GetSubCategory(categoryId: any) {
    ...
    const options1 = subCategoryList.map((subcategory: any) => {
      ...
      subcategory.label = (
        <div>
          <img src={subcategory.BillerLogo} height="20px" width="20px" />
          {subcategory.BillerName}{" "}
        </div>
      );
      return subcategory;
    });
  ...
  async function getGetOperatorDetails(number: Number) {
    ...
    if (operatorDetails?.billerid) {
      let result = subCategoryList.filter(
        (operator: any) => operator?.billerid == operatorDetails?.billerid
      );
      if (result.length > 0) {
  ...
  const payBillPayment = (event: any) => {
    ...
    if (billInformation?.validationid) {
      ...
      billInformation.ConnectionNumber =
        subCategory.billerParameters[0]?.ConnectionNumber; //billPayForm.ConnectionNumber;
  ...
  const handleClick = (index: number) => () => {
    setErrorBillerName("");
    setOperatorError("");
    localStorage.setItem("showBBPS", "false");
  ...
  async function FetchPrepaidPlan(billerid: Number, circleName: String) {
    setState((prev: any) => ({ ...prev, isLoading: true }));
    const fetchPrepaidPlan = await userService.allFetchPrepaidPlan({
      billerid: billerid,
  ...
  const handleOperatorChange = (e: any) => {
    ...
    currentSubCategory.billerParameters.map(
      (billerParameter: any, index: number) => {
        currentSubCategory.billerParameters[index].ConnectionNumber = null;
      }
    );
    setSubCategory(currentSubCategory);
    setBillerid(currentSubCategory?.billerid);
    if (currentSubCategory?.IsFluctuate) {
      setOperatorError(currentSubCategory.FluctMsg);
    }
    if (currentSubCategory?.IsBillerBBPS) {
      setIsBillerBBPS(currentSubCategory.IsBillerBBPS);
      localStorage.setItem("showBBPS", currentSubCategory.IsBillerBBPS);
    }
    currentSubCategory?.PartialPay === "N"
  ...
  const handlePlanTypeChange = (event: any) => {
    ...
    if (plantype) {
      let data = prepaidplanlist.filter(
        (prepaidplan: any) => prepaidplan.plan_category_name == plantype
      );
      setAllPlans(data);
    } else {
  ...
  const handleRechargeClick = (index: number) => {
    ...
    let selectedplan: any = allplans[index];
    fetch("https://utility.pinkytravels.com/api/UtilityAPI/ValidateRequest", {
      method: "POST",
      headers: {
              ...
              <ul className="nav secondary-nav">
                {categories &&
                  categories.map((category: any, index: number) => {
                    if (index < 7) {
                      return (
                        <li key={JSON.stringify(index)} className="nav-item">
                          {" "}
                          <div
                          ...
                          >
                            ...
                            >
                              <button
                                ...
                                className={
                                  category.PayID == currentCategory.PayID
                                    ? "accordion-button"
                            ...
                            >
                              <div className="accordion-body">
                                <h2 className="text-4 mb-3">
                                  {currentCategory.Payheading}
                                </h2>
                                {isNumberTrue ? (
                                  <form
                                    id="recharge-bill"
                                  ...
                                  <form
                                    id="recharge-bill"
                                    method="post"
                                    onSubmit={handleBillPaymentSubmit}
                                  >
                                    <div className="mb-3">
                                      <Select
                                        placeholder="Select Your Operator"
                                    ...
                                    {subCategory &&
                                      subCategory.billerParameters &&
                                      subCategory.billerParameters.map(
                                        (
                                          billerParameter: any,
                                          index: number
                                        ) => (
                                          <div className="mb-3">
                                            <input
                                              ...
                                              className="form-control"
                                              data-bv-field={
                                                billerParameter?.ConnectionNumber
                                              }
                                              required
                                              value={
                                                billerParameter?.ConnectionNumber
                                              ...
                                              onChange={(event: any) =>
                                                handleInputBillPaymentChange(
                                                  index,
                                                  event
                                                )
                                              }
                                              placeholder={
                                                billerParameter?.ParameterName
                                              }
                                              name={
                                                billerParameter?.ConnectionNumber
                                              }
                                    ...
                                    {billInformation?.validationid ? (
                                      <div className="d-grid">
                                        <div
                                          className="btn btn-primary w-100"
                                          onClick={payBillPayment}
                                        >
                                          Pay
                                        </div>
                                      </div>
                  ...
                  {isNumberTrue ? (
                    ...
                    >
                      <div className="mb-3">
                        <input
                          ...
                          value={rechargeForm.number}
                          placeholder={
                            currentCategory && currentCategory.DefaultMessage
                          }
                          onChange={handleInputChange}
                        />
                        {numberError && (
                          <span style={numErrorStyle}>{numberError}</span>
                        )}
                      </div>
                      <div className="mb-3">
                        {/* <Select
                      ...
                      <div className="input-group mb-3">
                        {" "}
                        <span className="input-group-text"></span>{" "}
                        <div onClick={handleShow} className="view-plans-link">
                          View Plans{" "}
                        </div>
                        <input
                          className="form-control"
                      ...
                      {planError && (
                        <span style={numErrorStyle}>{planError}</span>
                      )}
                      <div className="d-grid">
                        <button className="btn btn-primary">
                          Continue to Recharge
                    ...
                    >
                      <div className="mb-3">
                        ...
                        />
                        {errorBillerName && (
                          <span style={numErrorStyle}>{errorBillerName}</span>
                        )}
                      </div>
                      {subCategory &&
                        subCategory.billerParameters &&
                        subCategory.billerParameters.map(
                          (billerParameter, index) => (
                            <div className="mb-3">
                              <input
                                type="text"
                                className="form-control"
                                ...
                                required
                                value={billerParameter?.ConnectionNumber}
                                onChange={(event: any) =>
                                  handleInputBillPaymentChange(index, event)
                                }
                                placeholder={billerParameter?.ParameterName}
                                name={billerParameter?.ConnectionNumber}
                              />
                      ...
                      {isPartialPay ? (
                        <div className="mb-3">
                          <input
                            ...
                            readOnly
                            onChange={(event: any) =>
                              setChargeableAmount(event.target.value)
                            }
                            placeholder="Amount"
                            name="amount"
                          />
                      ...
                      {operatorError && (
                        <span style={numErrorStyle} className="abc">
                          {operatorError}
                        </span>
                      )}
                      {billInformation?.validationid ? (
          ...
          <div className="section pt-4 pb-3 ">
            <div className="container shadow-md">
              ...
              <div className="tab-content my-3" id="myTabContent">
                ...
                >
                  <p>
                    Instant Online mobile recharge Iisque persius interesset his
                    et, in quot quidam persequeris vim, ad mea essent possim
                    iriure. Mutat tacimates id sit. Ridens mediocritatem ius an,
                    eu nec magna imperdiet. Mediocrem qualisque in has. Enim
                    utroque perfecto id mei, ad eam tritani labores facilisis,
                    ullum sensibus no cum. Eius eleifend in quo. At mei alia
                    iriure propriae.
                  </p>
                  <p>
                    Partiendo voluptatibus ex cum, sed erat fuisset ne, cum ex
                    meis volumus mentitum. Alienum pertinacia maiestatis ne eum,
                    verear persequeris et vim. Mea cu dicit voluptua
                    efficiantur, nullam labitur veritus sit cu. Eum denique
                    omittantur te, in justo epicurei his, eu mei aeque populo.
                    Cu pro facer sententiae, ne brute graece scripta duo. No
                    placerat quaerendum nec, pri alia ceteros adipiscing ut. Quo
                    in nobis nostrum intellegebat. Ius hinc decore erroribus eu,
                    in case prima exerci pri. Id eum prima adipisci. Ius cu
                    minim theophrastus, legendos pertinacia an nam.{" "}
                    <a href="#">Read Terms</a>
                  </p>
                </div>
                <div
                  ...
                  id="billpayment"
                  role="tabpanel"
                  aria-labelledby="billpayment-tab"
                >
                  <p>
                    Partiendo voluptatibus ex cum, sed erat fuisset ne, cum ex
                    meis volumus mentitum. Alienum pertinacia maiestatis ne eum,
                ...
                <div
                  ...
                  id="why-quickai"
                  role="tabpanel"
                  aria-labelledby="why-quickai-tab"
                >
                  <p>
                    Cu pro facer sententiae, ne brute graece scripta duo. No
                    placerat quaerendum nec, pri alia ceteros adipiscing ut. Quo
            ...
            <div className="modal-body">
                  ...
                  >
                    ...
                    {prepaidcategory.map((prepaidcat: any, index: number) => {
                      return (
                        <option
                          key={JSON.stringify(index)}
                          value={prepaidcat.PlanCategory}
                        >
                          {prepaidcat.PlanCategory}
                        </option>
                      );
                    })}
                  </select>
              ...
              <div className="plans">
                <div className="row align-items-center">
                  <div className="col-4 col-lg-2 text-5 text-center">
                    Amount
                  </div>
                  ...
                  <div className="col-5 col-lg-1 text-5 text-center">
                    Category
                  </div>

                  <div className="col-5 col-lg-3 text-5 text-center">
                    Description
                  </div>
                ...
                {allplans &&
                  allplans.map((plan: any, index: number) => {
                    return (
                      ...
                      >
                        <div className="col-3 col-lg-2 text-5 text-primary text-center">
                          {plan.amount}
                          <span className="text-1 text-muted d-block"></span>
                        </div>

                        <div className="col-3 col-lg-2 text-3 text-center">
                          {plan.top_plan}
                        ...
                        <div className="col-3 col-lg-2 my-2 my-lg-0 text-1 text-muted">
                          {plan.plan_category_name}
                        </div>
                        <div
                          className="col-7 col-lg-2 my-2 my-lg-0 text-1 text-muted"
                        ...
                        <div className="col-5 col-lg-2 my-2 my-lg-0 text-end text-lg-center">
                          <button
                            ...
                            onClick={() => handleRechargeClick(index)}
                            type="submit"
                          >
                            Recharge
                          </button>
